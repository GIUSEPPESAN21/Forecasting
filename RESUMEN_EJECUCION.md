...
|---|---|---|---|
| F01 | `es_muy_lineal()` insatisfacible | **Corregido y verificado** | `forecasting_core/classification.py::allow_constant_level_methods`; `tests/test_classification.py`; Monte Carlo en `experiments/montecarlo_clasificacion.py` |
| F02 | Comparación no homogénea (fallback silencioso HW→Holt, SARIMA→ARIMA) | **Corregido y verificado** | `forecasting_core/validation.py::walk_forward` (exclusión explícita, nunca promedio sobre menos puntos); `tests/test_walk_forward_parity.py` |
| F03 | Fuga in-sample en el "MAPE histórico ajustado" | **Corregido y verificado** | Eliminado por construcción: `ModelSpec.forecast()` única ruta backtest/publicado; `tests/test_no_leakage.py` (perturbación programática del futuro) |
| F04 | `_make_predictor` despacha por subcadena, cae a `train[-1]` | **Corregido y verificado** | `forecasting_core/models.py::get_spec` (KeyError ante clave desconocida); `tests/test_model_registry.py` |
| F05 | Sesgo de selección: hiperparámetros elegidos sobre el mismo bloque reportado | **Corregido y verificado** | `forecasting_core/optimize.py` (partición tune/eval disjunta); `tests/test_hyperopt_no_bias.py` |
| F06 | Error Medio (ME) declarado, nunca implementado | **Corregido y verificado** | `forecasting_core/metrics.py::me`; `tests/test_metrics.py` |
| F07 | `template.tex` no compila, texto de plantilla, declaraciones vacías | **Corregido y verificado*** | `manuscript/template.tex` reescrito; ver nota de compilación abajo |
| F08 | Sección "Comparison with Prophet" sin protocolo, datos ni código | **Decisión de diseño aplicada: retirada y reemplazada** | `experiments/decision_prophet.md`; manuscrito Sección 3.6 (comparación contra AutoARIMA/ETS/Theta) |
| F09 | Afirmación central sin línea base (mejora vs. incumbente) | **Corregido, pendiente de dato externo** | `experiments/vs_incumbente.py` — corre sobre dataset sintético; acepta `--input` con el Excel real de Tuboplex cuando esté disponible |
| F10 | `\|ACF(12)\|>0.30` sin desestacionalizar (falsos positivos por tendencia) | **Corregido y verificado** | `forecasting_core/classification.py::seasonality_test` (STL + Kruskal-Wallis); Monte Carlo: 50.2%→1.4% medio |
| F11 | Test de tendencia no robusto a autocorrelación; ADF mal especificado | **Corregido y verificado** | `forecasting_core/classification.py::trend_test` (procedimiento secuencial ADF→GLSAR/deriva); Monte Carlo: 74.4%→8.9% medio |
| F12 | MAPE explota con demanda cero; sin naive/seasonal_naive; sin MASE | **Corregido y verificado** | `forecasting_core/metrics.py` (MASE primaria, MAPE excluye ceros); `MODEL_REGISTRY` incluye `naive`/`seasonal_naive` obligatorios |
| F13 | Límites de grid search inconsistentes; Holt/ARIMA/SARIMA sin tope real | **Corregido y verificado** | Parrillas coarse-to-fine acotadas (≤27 combos); ARIMA/SARIMA sin barrido (AICc vía `AutoARIMA`); `tests/test_model_registry.py::test_las_parrillas_estan_acotadas` |
| F14 | Cómputo redundante: walk-forward ejecutado 3-4 veces por sesión | **Corregido y verificado** | `forecasting_core/validation.py::walk_forward` (una sola pasada, agregado+detalle); `tests/test_walk_forward_parity.py::test_walk_forward_devuelve_agregado_y_detalle_en_una_sola_pasada` |
| F15 | Carga de datos frágil (meses, separadores, duplicados, huecos) | **Corregido y verificado** | `forecasting_core/data.py`; `tests/test_data_loading.py` (23 pruebas, cubre las 7 variantes) |
| F16 | `except Exception` sin registro; `warnings.filterwarnings("ignore")` global | **Corregido y verificado** | Sin filtro global en ningún módulo de `forecasting_core/`; logging configurable (`FORECASTING_LOG_LEVEL`) en `app.py`/`batch_cli.py` |
...
| F18 | Higiene de repositorio (debug=True, sin requirements/tests/licencia/semilla) | **Corregido y verificado** | `requirements.txt`, `LICENSE`, `app.py` con `debug=False` por defecto, semilla fija en todos los scripts de `experiments/` |
| F19 | Sin procesamiento por lotes multi-SKU | **Corregido y verificado** | `forecasting_core/batch.py`, `batch_cli.py`; `tests/test_batch_memory.py` (memoria acotada con `tracemalloc`) |
| F20 | Sin intervalos de predicción ni derivación de inventario | **Corregido y verificado** | `forecasting_core/intervals.py`, `forecasting_core/inventory.py`; `tests/test_batch_memory.py` (pruebas de intervalos y política) |
...
| F22 | Filtrado estructural afirmado en el paper, ausente en el código | **Decisión de diseño aplicada: (a) implementado** | `forecasting_core/models.py::eligible_specs`; manuscrito Sección 2.3 documenta el mecanismo real, con cada exclusión y su motivo |
| F23 | 5 de 11 fórmulas mal tipografiadas en la tesis | **Corregido y verificado** | Manuscrito Sección 2.5 usa notación LaTeX estándar verificada contra `forecasting_core/metrics.py`; las fórmulas de la tesis (documento separado) no se corrigieron porque no forman parte del alcance de este repositorio de código |
| F24 | Inconsistencias bibliográficas y atribuciones incorrectas | **Corregido y verificado** | 33 entradas reales en `manuscript/template.tex`, formato numérico ACS; `check_cites.py`: 0 citas rotas, 0 bibitems huérfanos |
| F25 | Tabla de tiempos no mide el módulo dominante; medición manual, n=1 | **Corregido y verificado** | `experiments/benchmark_tiempos.py` (`time.perf_counter()`, 5 repeticiones, incluye tuning); manuscrito Tabla 4 |
...
aplica, dentro de `manuscript/`) antes de enviar.
...
Ambos se corrigieron usando `honest_outer_estimate()` (`forecasting_core/optimize.py`),
...
`tests/test_hyperopt_no_bias.py::test_estimacion_externa_no_duplica_el_modelo_si_el_ganador_es_naive`.
...
- No se corrigieron las fórmulas matemáticas del documento de tesis
  ...
  (`manuscript/template.tex`) está dentro del alcance de este repositorio.
- No se validó contra los datos reales de Tuboplex (no disponibles en este
  entorno). `experiments/vs_incumbente.py --input <archivo>` es la ruta
