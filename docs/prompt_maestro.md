# PROMPT MAESTRO — Refactor y validación integral del motor de pronósticos Tuboplex
...
`ID` es la referencia canónica que debes usar en commits, tests y `CHANGELOG.md`. `Auditoría A` = referato de este chat; `Auditoría B` = artefacto "Auditoría Motor de Pronósticos Tuboplex" adjuntado por el usuario. `Fase` indica en qué fase de la §3 se resuelve.
...
| F09 | Afirmación central ("mejora significativamente la precisión") sin línea base: el error del método incumbente de Tuboplex nunca se calculó; sin benchmark ingenuo no hay forma de interpretar un MAPE aislado | C-09 | C-10 | 4 |
