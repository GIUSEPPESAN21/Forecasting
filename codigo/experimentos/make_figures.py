...
def fig1_flowchart():
    """Fig. 1: flujo metodologico de 9 etapas, con el bloque externo (F53)."""
    fig, ax = plt.subplots(figsize=(8.5, 7.0))
    ax.set_xlim(0, 10); ax.set_ylim(0, 8.6); ax.axis("off")

    stages = [
        (0.4, 7.2, "1. Data upload &\nvalidation", "#2E3A8C"),
        (0.4, 5.8, "2. Sequential structural\nclassification\n(seasonality → trend →\nstationarity)", "#2E3A8C"),
        (0.4, 4.2, "3. Candidate pool\n(structural filter)", "#3D4FA8"),
        (0.4, 2.7, "4. Walk-forward:\nTUNING block", "#5A6BC0"),
        (0.4, 1.2, "5. Walk-forward:\nEVALUATION block\n(held out from tuning)", "#B34700"),
    ]
    stage2 = [
        (5.4, 1.2, "6. OUTER block\n(held out from both\ntuning and evaluation;\nre-scores the winner)", "#7A1F5C"),
        (5.4, 2.7, "7. Ranking by MASE\n(naive/snaive always\nincluded)", "#B34700"),
        (5.4, 4.2, "8. Forecast + empirical\nprediction interval", "#177245"),
        (5.4, 5.8, "9. Inventory policy\n(safety stock, ROP)", "#177245"),
    ]

    def box(x, y, text, color):
        b = FancyBboxPatch((x, y - 0.6), 3.9, 1.2, boxstyle="round,pad=0.08,rounding_size=0.12",
                           linewidth=1.4, edgecolor=color, facecolor="white", zorder=2)
        ax.add_patch(b)
        ax.text(x + 1.95, y, text, ha="center", va="center", fontsize=8.4, color="#1a1a1a", zorder=3)
    ...
    arrow((2.35, 6.6), (2.35, 6.4))
    arrow((2.35, 5.2), (2.35, 4.8))
    arrow((2.35, 3.6), (2.35, 3.3))
    arrow((2.35, 2.1), (2.35, 1.8))
    arrow((4.3, 1.2), (5.4, 1.2))
    arrow((7.35, 1.8), (7.35, 2.1))
    arrow((7.35, 3.3), (7.35, 3.6))
    arrow((7.35, 4.8), (7.35, 5.2))

    ax.annotate("Never re-used to select\nhyperparameters", xy=(0.4, 2.7), xytext=(-1.55, 1.55),
               fontsize=7.0, color="#5A6BC0", ha="center",
               arrowprops=dict(arrowstyle="->", color="#5A6BC0", lw=0.8))
    ax.annotate("Never re-used to select\nthe winning method", xy=(5.4, 1.2), xytext=(3.6, -0.55),
               fontsize=7.0, color="#7A1F5C", ha="center",
               arrowprops=dict(arrowstyle="->", color="#7A1F5C", lw=0.8))
    ax.text(5.0, 8.25, "Structural classification (seasonality tested before trend), a strict\n"
                       "tuning/evaluation split, and a third outer block that keeps method\n"
                       "selection itself separate from the reported benchmark comparison.",
